...
public abstract class OnlineTermStore<T extends ReasonerTerm> implements VariableTermStore<T, RandomVariableAtom> {
    ...
    protected OnlineIterator<T> activeIterator;
    ...
    public OnlineTermStore(List<Rule> rules, AtomManager atomManager,
            OnlineTermGenerator<T, RandomVariableAtom> termGenerator) {
        ...
        for (Rule rule : rules) {
            if (!rule.isWeighted()) {
                if (warnRules) {
                    log.warn("Online term stores do not support hard constraints: " + rule);
                }
                continue;
            }
            ...
            if (((WeightedRule)rule).getWeight() < 0.0) {
                if (warnRules) {
                    log.warn("Online term stores do not support negative weights: " + rule);
                }
                continue;
            }
            ...
            if (!rule.supportsIndividualGrounding()) {
                if (warnRules) {
                    log.warn("Online term stores do not support rules that cannot individually ground (arithmetic rules with summations): " + rule);
                }
                continue;
            }
    ...
    public Iterator<T> noWriteIterator() {
        if (activeIterator != null) {
            throw new IllegalStateException("Iterator already exists for this OnlineTermStore. Exhaust the iterator first.");
        }
     ...
     */
    protected abstract OnlineIterator<T> getInitialRoundIterator();

    /**
     * Get an iterator that will read and write from disk.
     */
    protected abstract OnlineIterator<T> getCacheIterator();

    /**
     * Get an iterator that will not write to disk.
     */
    protected abstract OnlineIterator<T> getNoWriteIterator();
}
