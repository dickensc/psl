...
/**
 * A function that is the minimum of a set of terms.
 */
public class MinFunction extends AbstractFunction {

    public MinFunction(int maxSize, boolean squared, boolean mergeConstants) {
    ...
    public void add(float value) {
        constant = Math.min(constant, value);
        ((LinearFunction)terms[0]).setConstant(constant);
    }
    ...
    public void add(float coefficient, GeneralFunction term) {
        ...
        if (size == terms.length) {
            throw new IllegalStateException(
                    "More than the max terms added to the function. Min: " + terms.length);
        }
    ...
    public float getValue() {
        ...
        for (int i = 0; i < size; i++) {
            val = Math.min(val, terms[i].getValue() * coefficients[i]);
        }
    ...
    public float getValue(float[] values) {
        ...
        for (int i = 0; i < size; i++) {
            val = Math.min(val, coefficients[i] * values[i]);
        }
    ...
    public float getValue(GroundAtom replacementAtom, float replacementValue) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (term == replacementAtom) {
                val = Math.min(val, coefficient * replacementValue);
            } else {
                val = Math.min(val, coefficient * term.getValue());
            }
        }
